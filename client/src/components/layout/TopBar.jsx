        ...
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="relative hidden sm:block max-w-md flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-100 border-none rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:bg-white transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute top-1 right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-lg shadow-red-500/30 animate-pulse">
                  ...
                  <div className="max-h-80 overflow-y-auto">
                    {notifications.length > 0 ? notifications.map(n => (
                      <button
                        ...
                        className={`w-full text-left p-4 border-b border-slate-50 hover:bg-slate-50 transition-colors ${!n.isRead ? 'bg-indigo-50/50' : ''}`}
                      >
                        <div className="flex items-start gap-3">
                          <span className={`mt-0.5 px-2 py-0.5 text-[10px] font-semibold rounded-full ${typeColors[n.type] || 'bg-slate-100 text-slate-600'}`}>
                            {n.type}
                          </span>
